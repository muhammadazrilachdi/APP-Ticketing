    ...
    <table class="ui celled table" style="margin-top: 20px;">
        ...
        <tbody>
            <?php foreach ($priority_id as $p) : ?>
                <tr>
                    <td><?php echo $p['priority_id']; ?></td>
                    <td><?php echo $p['name']; ?></td>
                    <td>
                        <button class="ui button edit-button" data-priority-id="<?php echo $p['priority_id']; ?>" data-priority-name="<?php echo htmlspecialchars($p['name']); ?>">Edit</button>
    ...
    <div class="content">
        <form class="ui form" method="post" action="<?php echo site_url('admin/priority/process_tambah'); ?>"> <!-- Ganti dengan URL sesuai -->
            <div class="field">
                ...
                <input type="number" name="priority_id" placeholder="Masukkan Priority ID" required>
            </div>
            <div class="field">
    ...
    <div class="ui modal" id="editTicketModal_<?php echo $p['priority_id']; ?>">
        ...
        <div class="content">
            <form class="ui form" method="post" action="<?php echo site_url('admin/priority/process_edit'); ?>">
                <div class="field">
                    <label>Priority ID</label>
                    <input type="number" name="priority_id" value="<?php echo htmlspecialchars($p['priority_id']); ?>" placeholder="Masukkan Priority ID" required>
                </div>
                <div class="field">
                    <label>Nama Priority</label>
